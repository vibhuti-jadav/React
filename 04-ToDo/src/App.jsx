       ...
       <div className='bg-cyan-900 rounded w-2xl my-5 gap-5 flex flex-col mx-auto'>
        {
          todo.map((ele) => (
              <div key={ele.id} className='flex gap-x-50 p-3'>
                  ...
                  <button className='text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium  text-sm px-5 py-2.5 text-center p-2 rounded-md' onClick={() => handleUpdate(ele.id)}  >{ele.status ? "Completed" : "not Completed" }</button>
